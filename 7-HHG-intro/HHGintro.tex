...
% Complex space-time electron dynamics in the weak relativistic regime

% In this part we move to longer lambda and higher I, and explore the use of tailored laser fields, in particular to compensate for the effects of the Lorentz force on the recollision dynamics. We focus on HHG as a physical process. Thus we start with a brief review of it.
...
%\noindent
%(Strangely enough, this is one of only a handful of open implementations of this twenty-year-old method~\cite{rhyno_thesis, rhyno_code, hhgmax_thesis, hhgmax_code, slimp_paper, ClassSTRONG_paper}.)




%\section{Why harmonic generation?}
%\label{sec:why-hhg}
%Before we plunge in to the mechanisms behind the generation of high harmonics, it is worth looking at the motivations for studying HHG in the first place.
